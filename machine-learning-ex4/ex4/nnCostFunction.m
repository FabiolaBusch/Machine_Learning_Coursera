...
a2 = [ones(size(a2,1),1) a2]; % size 5000x26
z3 = a2*Theta2';
...
J_ik = zeros(m,num_labels);

for i=1:m
  for k =1:num_labels  
    J_ik(i,k)= -y_new(k,i)*log(h(i,k)) - (1-y_new(k,i))*log(1-h(i,k));
  end
end
...
J = 1.0/m * sum(sum(J_ik))+(lambda/(2.0*m))*(reg1+reg2);
